...
import (
	...
	"omnilogger/config"
	"omnilogger/model"
	drivers "omnilogger/pkg/drivers"
	"os"
	"testing"
	"time"
)
...
func (m *MockDriver) FormatLog(messageData model.MessageData) (string, error) {
...
func TestFileDriver(t *testing.T) {
	...
	driver, err := drivers.NewFileDriver(file.Name())
	if err != nil {
...
func TestFileDriverWithFormatting(t *testing.T) {
	...
	// Verify log fields
	if logEntry["level"] != string(omnilogger.INFO) {
...
func TestOmniLogger(t *testing.T) {
	...
	config := config.Config{
		LogLevels: map[config.LogLevel]bool{
			omnilogger.DEBUG: true,
			omnilogger.INFO:  false,
			omnilogger.WARN:  false,
			omnilogger.ERROR: true,
		},
	}

	// Create a new OmniLogger with the mock driver
	logger := omnilogger.NewOmniLogger(config, nil, mockDriver)
...
func TestLoadConfig(t *testing.T) {
	...
	if !config.LogLevels[omnilogger.DEBUG] {
		t.Error("expected DEBUG to be enabled")
	}
	if config.LogLevels[omnilogger.INFO] {
		t.Error("expected INFO to be disabled")
	}
	if !config.LogLevels[omnilogger.WARN] {
		t.Error("expected WARN to be enabled")
	}
	if !config.LogLevels[omnilogger.ERROR] {
...
func TestCLIDriver_FormatLog_NoContext(t *testing.T) {
	...
	expectedLog := "[" + string(omnilogger.INFO) + "] timestamp: " + timestamp + "  trace: file.go:123  msg : Test message without context "
...
func TestCLIDriver_FormatLog_WithContext(t *testing.T) {
	...
	// Check if the formatted log contains the correct data
	expectedLog := "[" + string(omnilogger.INFO) + "] timestamp: " + timestamp + " transaction_id: tx123 user_id: user456 key: value  trace: file.go:123  msg : Test message with context "
	if formattedLog != expectedLog {
		t.Errorf("expected formatted log to be '%s', got '%s'", expectedLog, formattedLog)
	}
}
